...
class Product {
    public static function getAll(){
            ...
            $req = $db->query('SELECT * FROM product order by LOWER(proName)');
    ...
    public static function getProductByOffset($offset, $itemPerPage){
        $list = [];
        $db = Database::getInstance();
        $req = $db->query('SELECT * FROM product ORDER BY LOWER(proName) LIMIT '.$offset.', '.$itemPerPage);
        foreach($req->fetchAll() as $rc) {
            $list[] = $rc;
        }
        return $list;
    ...
    public static function searchProduct($proType, $proBrand, $sortBy, $offset, $itemPerPage){
        $list=[];
        $db = Database::getInstance();
        ...
        $prepareSql = $prepareSql.' LIMIT '.$offset.', '.$itemPerPage;
        //echo $prepareSql;
    ...
    public static function searchProductTotal($proType, $proBrand, $sortBy, $offset, $itemPerPage){
        $list=[];
        $db = Database::getInstance();
        $prepareSql ="SELECT count(*) as total FROM product";
        if($proType !='' || $proBrand != 0){
            $prepareSql = $prepareSql." WHERE 1 = 1 ";
            if($proType != ''){
                $prepareSql = $prepareSql." AND typeCode = '".$proType."'";
